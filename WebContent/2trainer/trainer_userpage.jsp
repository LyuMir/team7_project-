...
List<PhotoBean> photoBean = (List<PhotoBean>) request.getAttribute("photos");
...
boolean zyn = false;
if (zyn2 != null) {
	zyn = true;//찜이 비어있으면 ~~!!! 트루!! //있으면 false인데 왜 로그인 안하면 false가 나오느냐?? 안에 아무것도없는데?!
}
...
if (photoBean != null && photoBean.size() > 0)
	picSource1 = "Files/trainer/" + tser.get(0).getId() + "/" + photoBean.get(0).getPicture();
...
<main class="mainwrap_club">
	<section class="club_right">
		...
		<article class="left_main">
			...
			<div class="profile_name">
				<h3><%=tser.get(0).getTname()%>
					코치
				</h3>
				<hr>
				<span class="a_text"> <%=tser.get(0).getTbigtext()%></span> <span>
				</span>
			</div>
			<div class="zzim" style="width: 100%;">
				<%
				if (zyn) {//트루면!! 찜이 비어있으면!!! //로그인안할시에 
				%>
				<button data-id2="<%=tser.get(0).getNo()%>" class="hero__btn1"
					onclick="zzimThis2_C(this)">
					찜 취소 <img src="img/heart_and_star/heart34.png">
				...
				<% }else{%>
				<button data-id2="<%=tser.get(0).getNo()%>" class="hero__btn1"
					onclick="zzimThis2(this)">
					찜하기 <img src="img/heart_and_star/heart034.png">
				</button>
	...
	<section class="">
		<article class="main_head">
			<!-- 사진 크게 넣을거임. ...사진 될까? -->
			<img class="ImageForModal" src="<%=picSource1%>"
				onclick="ImageClickFunction(this)">
		...
		<article class="mainwrap">
			...
			<div class="info">
				...
				<div class="detail_pan">
					<img src="img/time.png" class="littleimg">
					<p class="label">수업지역</p>
				</div>




				<p style="font-size: 13px; margin: 0 0;">
					●<%=tser.get(0).getTwhere()%></p>




				<div class="detail_pan">
					<img src="img/effect.png" class="littleimg">
					<p class="label">수업효과</p>
				</div>
				...
				<div class="detail_pan">
					<img src="img/when.png" class="littleimg">
					<p class="label">수업가능시간</p>
				</div>
			...
			<div class="coachprofile info">
				...
				<div class="profile_name">
					<h3 class="hero__info"><%=tser.get(0).getTname()%>
						코치
					</h3>
			...
			<div class="career info">
				<p class="hg2" id="coach2">코치경력사항 * 활동내용</p>
				<br>
				<p>
			...
			<div class="cert info">
				<p class="hg" id="program">프로그램소개</p>
				<br>
				<p>
			...
			<div class="cert info">
				<br>
				<p class="hg" id="review">후기 및 문의사항</p>
				<br>
				<!-- 포스트는 여기 뒤에.  -->
				<%
				...
				<div class="posts">
					...
					<article>
						...
						<div class="deleter fright">
							<img style="width: 10px; margin-top: 5px; cursor: pointer;"
								src="img/icons/close33.png" onclick="deletethispost(this)">
							<form action="delete.post" style="display: none;">
								<input name="tpostno" value="<%=posts.get(i).getNo()%>">
							</form>
						</div>
						<%
						}
						...
						%>
						<div class="post_like fright" onclick="tpost_z_cancel(this)"
							data-no="<%=posts.get(i).getNo()%>">
							<img src="img/heart_and_star/heart35.png"> <span><%=zcount%></span>
						</div>
						<%
						} else {
						%>
						<div class="post_like fright" onclick="tpost_z(this)"
							data-no="<%=posts.get(i).getNo()%>">
						...
						%>
						<div class="post_writer fright">
							by
							<%=posts.get(i).getWriter()%></div>
						<%
						if (pcon == 1) {
						%>
			...
			<p class="container1">
				※ 1회 체험에 관한 문의사항을 자유롭게 남겨주세요 ^^<br> ※코치님이 확인후 연락을드립니다!<br>
			...
			<div id="wf1">
				<article class="new_write">
					<form id="wForm1" method="post" action="trainerPost.post">
						<div>
							<input id="imtitle1" type="text" name="title" class="inputclass"
								placeholder="제목"> <input type="hidden" name="trainerid"
								value="<%=tser.get(0).getNo()%>"> <input type="hidden"
								name="pkind" value="문의사항"> <input type="hidden"
								name="pcon" value="" id="pco1"> <select
								class="selectclass" style="width: 25%;">
			...
			<div id="wf2">
				<article class="new_write">
					<form id="wForm2" method="post" action="trainerPost.post">
						...
						<textarea id="imtext2"
							placeholder="트레이너에게 원하시는게 있나요? 감동받은 사항은요? 후기를 남겨주세요!"
